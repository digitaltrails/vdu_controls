...
def format_solar_elevation_description(elevation: SolarElevationKey) -> str | None:
    ...
    direction_text = translate('eastern-sky') if elevation.direction == EASTERN_SKY else translate('western-sky')
...
def translate(source_text: str, disambiguation: str = None):
    """
    ...
    1) Generate template file from this code, for example for French:
       ALWAYS BACKUP THE CURRENT .ts FILE BEFORE RUNNING AN UPDATE - it can go wrong!
           pylupdate5 -tr-function translate  vdu_controls.py -ts translations/fr_FR.ts
...
LOCALE_TRANSLATIONS_PATHS = (Path(CONFIG_DIR_PATH).joinpath('translations'),
                             Path("/user/share/vdu_controls/translations"),)
...
class PresetWidget(QWidget):
    def __init__(self,
        ...
        delete_button.setToolTip(translate('Delete this preset.'))
...
class PresetsDialog(QDialog, DialogSingletonMixin):
    ...
    def __init__(self, main_window: 'MainWindow', main_config: VduControlsConfig) -> None:
        ...
        presets_panel_title = QLabel(translate("Presets"))
    ...
    def create_preset_content_controls(self) -> QWidget:
        ...
        for count, section in enumerate(self.base_ini.data_sections()):
            ...
            group_box.setToolTip(translate("Choose which settings to save for {}").format(section))
...
def main():
    ...
    translator = initialise_locale_translations(app)
    ...
    if rc == EXIT_CODE_FOR_RESTART:
        ...
        if not restart_status:
            ...
            dialog.setText(translate("Restart of {} failed.  Please restart manually.".format(app.arguments()[0])))
            dialog.setInformativeText(translate("This is probably because {} is not"
